...
tape('maintenance html response', function(t) {
    var api = nock('https://pokevision.com')
        ...
        .reply(200, function(uri) {
            ...
            return '<html><h1>Maintenance</h1><p>Pokevision API is undergoing maintenance.</p></html>';
        });

    pokegoScan(coords, function(err) {
        t.equal(err && err.message, 'Pokevision API is undergoing maintenance. Everything should be completed shortly.');
        t.end();
    });
});
...
tape('scanning invalid coordinates', function(t) {
    ...
    pokegoScan(invalidCoords, function(err) {
        t.equal(err && err.message, 'Unknown response received from PokeVision API.');
        t.end();
    });
});
