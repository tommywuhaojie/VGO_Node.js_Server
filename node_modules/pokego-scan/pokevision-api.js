...
function getErrorFromHtmlResp(html) {
    ...
    if (html.indexOf('{disabled}') !== -1) {
        ...
        err = new Error('Pokevision API is undergoing maintenance. Everything should be completed shortly.');
    } else {
        err = new Error('Unknown response received from PokeVision API.');
    }
...
function getErrorFromJsonResp(json) {
    ...
    if (json.message.indexOf('{disabled}') !== -1) {
        ...
        err = new Error(json.message || 'Unknown invalid response received from PokeVision API.');
    }
